...
SpaceInvaders.PlayerContext = function (game) {
  ...
  this.getAlienStates = function () {
    return alienStates;
  };
  this.getShieldStates = function () {
    return shieldStates;
  };
  ...
  this.setAlienStates = function (newStates) {
    alienStates = newStates;
  };
  this.setShieldStates = function (newStates) {
    shieldStates = newStates;
  };

  this.addScore = function (additionalScore) {
...
SpaceInvaders.Game = function () {
  ...
  this.getPlayer1Context = function () {
    return player1Context;
  };
  this.getPlayer2Context = function () {
  ...
  this.setHiScore = function (newScore) {
    hiScore = newScore;
  };
  this.setPlayerCount = function (newCount) {
...
SpaceInvaders.SpriteEntity = function (game) {
  ...
  this.getClipX = function () {
    return clipX;
  };
  this.getClipY = function () {
    return clipY;
  };
  ...
  this.setClipX = function (newClip) {
    clipX = newClip;
  };
  this.setClipY = function (newClip) {
    clipY = newClip;
  };
...
SpaceInvaders.MovableSpriteEntity = function (game) {
  ...
  this.setDirectionX = function (newDirection) {
    directionX = newDirection;
  };
  this.setDirectionY = function (newDirection) {
...
SpaceInvaders.AlienShotEntity = function (game, scene) {
  ...
  this.getProgressTicks = function () {
...
SpaceInvaders.AnimatedMovableSpriteEntity = function (game) {
  ...
  this.getAnimationStepSize = function () {
    return animationStepSize;
  };
  this.getAnimationFrameIndex = function () {
    return animationFrameIndex;
  };
  this.getAnimationFrames = function () {
...
SpaceInvaders.TextEntity = function (game) {
  ...
  this.getBlinkCount = function () {
    return blinkCount;
  };
  this.getBlinkFrequency = function () {
  ...
  this.setFillStyle = function (newStyle) {
    fillStyle = newStyle;
  };
  ...
  this.setBlinkCount = function (newCount) {
    blinkCount = newCount;
  };
  this.setBlinkFrequency = function (newFreq) {
  ...
  this.update = function (dt) {
    ...
    if (avatar.getDirectionX() == -1) {
      if (alienLeftBoundsDetector.collides(avatar)) {
        ...
        avatar.setX(
          alienLeftBoundsDetector.getX() +
            2 * alienLeftBoundsDetector.getExtentX()
        );
      }
    ...
    if (avatar.isEnabled()) {
      ...
      if (alienShots[0].isReadyToBeFired() && alienRollingShotLock <= 0) {
        ...
        if (alienIdx != -1) {
          alienShots[0].setX(
            aliens[alienIdx].getCenterX() - alienShots[0].getExtentX()
          );
          alienShots[0].setY(
            aliens[alienIdx].getY() + aliens[alienIdx].getHeight()
          );
          alienShots[0].fire();
        }
    ...
    if (activeAlienCount > 1) {
      if (avatar.isEnabled() && alienShots[1].isReadyToBeFired()) {
        ...
        alienPlungerShotColumnIndice =
          alienPlungerShotColumnIndice % this.ALIEN_SHOT_INDICE_COUNT;
        ...
        for (n = 4; n >= 0; n--) {
          ...
          if (aliens[idx].isVisible()) {
            ...
            alienShots[1].setX(
              aliens[idx].getCenterX() - alienShots[1].getExtentX()
            );
    ...
    if (
      avatar.isEnabled() &&
      flyingSaucer.isVisible() == false &&
      alienShots[2].isReadyToBeFired()
    ) {
      ...
      if (flyingSaucerCounter <= 0 && activeAlienCount >= 8) {
        ...
        alienSquigglyShotColumnIndice =
          alienSquigglyShotColumnIndice % this.ALIEN_SHOT_INDICE_COUNT;
        ...
        for (n = 4; n >= 0; n--) {
          ...
          if (aliens[idx].isVisible()) {
            // assign the position of the plunger shot based on the nearest alien.
            alienShots[2].setX(
              aliens[idx].getCenterX() - alienShots[2].getExtentX()
            );
    ...
    if (avatarLaser.isVisible()) {
      ...
      if (avatarLaser.collides(topOutOfBoundsDetector)) {
        ...
        avatarLaser.setY(
          topOutOfBoundsDetector.getY() +
            topOutOfBoundsDetector.getExtentY() * 2
        );
        ...
        for (n = 0; n < aliens.length; n++) {
          if (avatarLaser.collides(aliens[n])) {
            ...
            var newStepSize =
              aliens[0].getStepSize() - this.ALIEN_STEP_DECREMENT_SIZE;
